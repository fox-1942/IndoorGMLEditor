...
public class InterLayerConnectionDialog extends JDialog {
     ...
     */
    /*
     * public static void main(String[] args) { try { InterLayerConnectionDialog dialog = new InterLayerConnectionDialog();
     * dialog.setDefaultCloseOperation(JDialog.DISPOSE_ON_CLOSE); dialog.setVisible(true); } catch (Exception e) { e.printStackTrace(); } }
     */
    /**
     * Create the dialog.
     */
    public InterLayerConnectionDialog(MainFrame mainFrame, ProjectFile project,
            ArrayList<State> statesEnd1, ArrayList<State> statesEnd2, SpaceLayer spaceLayerEnd1,
            SpaceLayer spaceLayerEnd2) {
            ...
            getContentPane().add(buttonPane, BorderLayout.SOUTH);
            {
                ...
                okButton.addActionListener(new ActionListener() {
                    public void actionPerformed(ActionEvent arg0) {
                        ...
                        ArrayList<InterLayerConnection> ilcMember = project.getMultiLayeredGraph()
                                .getInterEdges().get(0).getInterLayerConnectionMember();
                        for (State state1 : statesEnd1) {
                            for (State state2 : statesEnd2) {
                                ...
                                SpaceLayer[] connectedLayers = new SpaceLayer[] { spaceLayerEnd1,
                                        spaceLayerEnd2 };
    ...
    private JTable getTable_End1() {
        if (table_End1 == null) {
            ...
            table_End1.setModel(new DefaultTableModel(new Object[][] { { null, null, null }, },
                    new String[] { "SpaceLayer", "State", "Name" }) {
    ...
    private JTable getTable_End2() {
        if (table_End2 == null) {
            ...
            table_End2.setModel(new DefaultTableModel(new Object[][] { { null, null, null }, },
                    new String[] { "SpaceLayer", "State", "Name" }) {
                Class[] columnTypes = new Class[] { String.class, String.class, String.class };
    ...
    private JComboBox getComboBox_Topology() {
        if (comboBox_Topology == null) {
            ...
            comboBox_Topology.setModel(new DefaultComboBoxModel(new String[] { "CONTAINS",
                    "OVERLAPS", "EQUALS", "WITHIN", "CROSSES", "INTERSECTS" }));
    ...
    private void updateTableModel() {
        ...
        for (State state : statesEnd1) {
            model.addRow(new Object[] { spaceLayerEnd1.getGmlID(), state.getGmlID(),
                    state.getName() });
        }
        ...
        for (State state : statesEnd2) {
            model.addRow(new Object[] { spaceLayerEnd2.getGmlID(), state.getGmlID(),
                    state.getName() });
        }
    }
}

...
public class CellSpaceBoundaryBuilder {

	private CanvasPanel panel;
	private IndoorFeatures indoorFeatures;
	
	public CellSpaceBoundaryBuilder(CanvasPanel panel, IndoorFeatures indoorFeatures) {
		this.panel = panel;
	...
	public void build() {
		...
		for (int p = 0; p < cellSpaceOnFloorList.size(); p++) {
			...
			for (int i = 0; i < cellSpaceMember.size(); i++) {
				CellSpace cellSpace = cellSpaceMember.get(i);
				...
				for (int j = 0; j < connects.size(); j++) {
					...
					Transition transition = connects.get(j);
					...
					if (duality.equals(state1)) {
						targetState = state2;
					} else {
						targetState = state1;
					}
					
					CellSpace targetCellSpace = targetState.getDuality();
					if (!cellSpaceMember.contains(targetCellSpace)) {
						continue;
					}
					
					for (int k = 0; k < lineStringElements.size(); k++) {
			            LineString ls = lineStringElements.get(k);
			            
			            if (GeometryUtil.isIntersectsLineString(ls, transition.getPath())) {
			            	for (LineString otherLS : targetCellSpace.getLineStringElements()) {
			            		if (GeometryUtil.isIntersectsLineString(otherLS, transition.getPath())) {
			            			CellSpaceBoundary newBoundary = createCellSpaceBoundaryForJSK(ls, otherLS, cellSpace, targetCellSpace, cellSpaceBoundaryOnFloor);
		                        	if (newBoundary != null) {
		                        		newBoundary.setDuality(transition);
			                        	transition.setDuality(newBoundary);
		                        	}
					...
					if (transition.getDuality() == null) {
						System.out.println(transition.getGmlID() + " don't have duality!!");
					}
				}
			}
		}
